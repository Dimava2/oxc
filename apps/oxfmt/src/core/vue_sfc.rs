...
pub(super) fn parse_script_blocks(source_text: &str) -> Vec<VueScriptBlock> {
    ...
    while let Some(open_start) = find_script_open_tag(source_text, pointer) {
        ...
        let lang = extract_lang_attribute(open_tag_content).map(ToString::to_string);

        let Some(close_rel) = source_text[open_end..].find("</script>") else {
            break;
        };
        let content_end = open_end + close_rel;
        blocks.push(VueScriptBlock { content_start: open_end, content_end, lang });

        pointer = content_end + "</script>".len();
    }

    blocks
}

fn find_script_open_tag(source_text: &str, from: usize) -> Option<usize> {
    let mut pointer = from;
    while let Some(rel_idx) = source_text[pointer..].find("<script") {
        let idx = pointer + rel_idx;

        // Skip tags inside HTML comments.
        if is_inside_html_comment(source_text, idx) {
            pointer = idx + "<script".len();
            continue;
        }

        let boundary = source_text[idx + "<script".len()..].chars().next()?;
        if matches!(boundary, '>' | ' ' | '\t' | '\n' | '\r') {
            return Some(idx);
        }
